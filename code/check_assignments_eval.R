...
# Data cleaning ----
df <- df %>% mutate(treatment_group = factor(survey_group, 
                                             levels = c(0,1,2),
                                             labels = c('Control', 'PSA', 'Concerns')))

# select only users assigned to one of the three evaluation treatment groups
df <- df[which(!is.na(df$survey_group)),]

# exclude those who were assigned to survey_group 2 but did not proceed to get the treatment_id assigned
df <- df %>% filter(!(survey_group == 2 & is.na(treatment_id)))
df <- df %>% mutate(vax_status = tolower(vax_status))
df <- df %>% filter((vax_status == 'no'))

# re-sort concerns based on templates
df$concern_id <- as.numeric(df$concern_id) # TODO: recode to mutate
...
table(df$concern_id2, df$treatment_id2)
table(df$concern_id3, df$treatment_id3)
...
df[is.na(df$willingness_2), "willingness_2"]
...
lm_robust(response~treatment_group, data = df)
lm_robust(response~treatment_group + factor(willingness) + factor(get_vaccinated), data = df)
lm_lin(response~treatment_group, covariates = ~ factor(willingness) + factor(get_vaccinated), data = df)
...
lm_robust(share_post_numeric~treatment_group, data = df)
...
lm_robust(share_true_numeric~treatment_group, data = df)
...
lm_robust(share_false_numeric~treatment_group, data = df)
